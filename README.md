...
| 영역 | 동작 |
| --- | --- |
| 시계 | 잡고 드래그하면 이동. 마우스가 떠나면 박스가 흐려지고 펼친 항목은 12초 뒤 자동으로 접힘 |
| 타이머 | **클릭** 시작 / 중지, **길게 누르기(0.6초)** 리셋. 마우스를 올리면 목표(분) 입력이 나타나고 도달 시 알림 |
| 사용량 | 막대만 표시. **클릭**하면 %, 리셋 시각, 토큰 수치. 토큰 막대는 7일 총량 위에 오늘 분량을 밝게 겹쳐 표시 |
| 알림 | 최신(미읽음 우선) 한 줄. **클릭**하면 목록, 항목 **클릭**하면 본문 전체, × 로 삭제 |
